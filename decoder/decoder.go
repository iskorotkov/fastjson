...
func New(typ reflect.Type) Decoder {
	...
	kind := typ.Kind()
	if int(kind) >= len(decodersByKind) {
...
func decodeInt(value reflect.Value, tokens *tokenizer.Tokenizer) {
	...
	integer, err := strconv.ParseInt(types.BytesToString(token.Literal), 10, 64)
	if err != nil {
...
func decodeUint(value reflect.Value, tokens *tokenizer.Tokenizer) {
	...
	integer, err := strconv.ParseUint(types.BytesToString(token.Literal), 10, 64)
	if err != nil {
		panic(&LiteralParseError{
			Err:   err,
			Token: token,
...
func decodeFloat(value reflect.Value, tokens *tokenizer.Tokenizer) {
	...
	float, err := strconv.ParseFloat(types.BytesToString(token.Literal), 64)
	if err != nil {
		panic(&LiteralParseError{
			Err:   err,
			Token: token,
...
func decodeDecoder(value reflect.Value, tokens *tokenizer.Tokenizer) {
	...
	value.SetString(types.BytesToString(token.Unquote()))
}
	...
	return func(value reflect.Value, tokens *tokenizer.Tokenizer) {
		...
		case tokenizer.TokenTypeObjectStart:
			...
			for {
				...
				key := types.BytesToString(token.Unquote())
...
func structDecoder(typ reflect.Type) Decoder {
	...
	for i := range typ.NumField() {
		field := typ.Field(i)
		name := metadata.JSONFieldName(field)
		dec := New(field.Type)
	...
	return func(value reflect.Value, tokens *tokenizer.Tokenizer) {
		...
		for {
			...
			name := types.BytesToString(token.Unquote())
...
func decodeTimeDuration(value reflect.Value, tokens *tokenizer.Tokenizer) {
	...
	case tokenizer.TokenTypeLiteral:
		dur, err := strconv.ParseInt(types.BytesToString(token.Literal), 10, 64)
		if err != nil {
			panic(&LiteralParseError{
				Err:   err,
				Token: token,
	...
	case tokenizer.TokenTypeQuotedLiteral:
		dur, err := time.ParseDuration(types.BytesToString(token.Unquote()))
		if err != nil {
			panic(&LiteralParseError{
				Err:   err,
				Token: token,
